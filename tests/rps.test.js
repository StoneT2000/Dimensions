...
class RockPaperScissorsDesign extends Dimension.Design{
	async initialize(match, config) {
		...
		let state = {
			maxRounds: config.bestOf, // we will store the number of rounds of rock paper scissors this game will run
	...
	async update(match, commands, config) {
		...
		// You need to return a promise that then resolves with a MatchStatus. Dimensions uses this MatchStatus to determine
		// how to proceed
		
		return new Promise((resolve) => {
			// We have that each agent will give us a command that is one of 'R', 'P', or 'S' indicating Rock, Paper, Scissors

			if (!commands.length) resolve (MatchStatus.RUNNING)
			let winningAgent;
			...
			// update the match state if it wasn't a tie
			...
			// if rounds reaches maxrounds, we resolve with MatchStatus.FINISHED
			if (match.state.rounds === match.state.maxRounds) {
				resolve(MatchStatus.FINISHED);
			}
			// otherwise we proceed as normal by resolving MatchStatus.RUNNING
			resolve(MatchStatus.RUNNING);
		});
	}
	async getResults(match, config) {
