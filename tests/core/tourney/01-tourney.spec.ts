...
import 'mocha';
import { Logger, MatchEngine, Match, Agent, Design, Tournament } from '../../../src';
import { createRoundRobinTourney, createLadderTourney, createElimTourney } from './utils';
...
describe('Testing Tournament Core', () => {
  ...
  const testbot = {file: './tests/kits/js/normal/paper.js', existingID: 'test'};
  ...
  describe("Test Configurations", () => {
    it("should adopt same match configs as dimension defaults", () => {
      ...
      expect(tourney.getConfigs().defaultMatchConfigs).to.eql(d.configs.defaultMatchConfigs);
      expect(tourney.getConfigs().loggingLevel).to.equal(d.configs.loggingLevel);
  ...
  describe("Test functions", () => {
    it("should get player stat", async () => {
      ...
      let { user, playerStat } = await tourney.getPlayerStat(testbot.existingID);
