...
describe('Test Logger', () => {
  ...
  it('Level ALL', () => {
    ...
    expect(consoleSpy).to.have.been.calledWith(colors.red('[SYSTEM]') + ' (Log) -', 'hello-sys');
    expect(consoleSpy).to.have.been.calledWith(colors.gray('[DETAIL]') + ' (Log) -', 'hello-det');
    expect(consoleSpy).to.have.been.calledWith(colors.red('[SYSTEM I/O]') + ' (Log) -', 'hello-sysIO');
  });

  it('Level ALL, printing bars', () => {
    initBars(Logger.LEVEL.ALL);
    expect(consoleSpy).to.have.been.calledWith(log.bar(colors.red('[SYSTEM I/O]')));
    ...
    expect(consoleSpy).to.have.been.calledWith(log.bar(colors.yellow('[WARN]')));
    expect(consoleSpy).to.have.been.calledWith(log.bar(colors.blue('[INFO]')));
    expect(consoleSpy).to.have.been.calledWith(log.bar(colors.gray('[DETAIL]')));
    log.important("hello-important");
    log.importantBar();
    log.bar();
    expect(consoleSpy).to.have.been.calledWith(log.bar(colors.red('[IMPORTANT]')));
    expect(consoleSpy).to.have.been.calledWith(colors.red('[IMPORTANT]') + ' (Log) -', 'hello-important');
